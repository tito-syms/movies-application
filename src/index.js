...
function updateMovies() {
    getMovies().then((movies) => {
        ...
        movies.forEach(({title, rating, id}) => {
            movieTitles +=
                `<div><p><h4>${title}</h4>Rating: ${rating}<br><span class="glyphicon glyphicon-edit tooltiptext" id="edit${id}"></span>     <span class="glyphicon glyphicon-trash tooltiptext" id="trash${id}"></span></p></div>`;
            $('#movies').html(movieTitles);
            console.log(`id#${id} - ${title} - rating: ${rating}`);
        });
        makingMoviesGarbage(movies);
    }).catch((error) => {
...
//Delete Movie
function makingMoviesGarbage(movies){
movies.forEach(({title, rating, id}) => {
